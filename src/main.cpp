...
int main(int argc, char** argv)
{
	...
	paths.push_back(args.input);
	ZrawProcessingModel model;
	model.InputFilePath_add(paths);
	model.OutputFolderPath_set(args.output);

	IConsoleOutput console;
	IProgressBar progress;
	ZRawProcess p(model, console, progress);
	p.Process();
